getBoxPlots <- function(x,x2,titleStr,resultsPath,org,omic,colorValues,reps){ 
lcpm1     <- cpm(x, log = TRUE)
lcpm2     <- cpm(x2, log = TRUE)
nsamples  <- ncol(x)
colors    <- c()
for (i in 1:length(reps)){
  colors <- c(colors,rep(colorValues[i],reps[i]))
}
#colors    <- colorValues#brewer.pal(nsamples, "Paired")
